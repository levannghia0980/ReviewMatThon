...
class CapCutClient:
    ...
    def __init__(
        ...
        :param cookie: Optional cookie string (e.g. sessionid) for authentication.
        """
    ...
    def build_tts_new_request(
        ...
        if self.cookie:
            headers["cookie"] = self.cookie if "sessionid=" in self.cookie else f"sessionid={self.cookie}"
    ...
    def list_voices(
        ...
        path = catalog_path
        if not path:
            p1 = Path(__file__).parent / "Voice.json"
            p2 = Path(__file__).parent.parent / "Voice.json"
